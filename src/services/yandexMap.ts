...
export const generateGeoData = (points: IPoint[]): Record<string, unknown> => ({
  ...
  features: points.map((point, index) => ({
    ...
    options: {
      ...
      balloonAutoPanMargin: [-520, 200, 640, -350], //верхний, правый, нижний и левый
...
export const generateRouteGeoData = (
  points: IPoint[]
  ...
  features: points.map((point, index) => ({
    ...
    options: {
      ...
      balloonAutoPan: true,
      balloonPanelMaxMapArea: 0,
      balloonAutoPanMargin: [-150, 300, 640, -550], //верхний, правый, нижний и левый
...
const init = ({ points }: IInitMapProps) => {
  let referencePoints = points?.routePoints.map(
